...
export function LinksSection() {
  return (
    <div className="relative">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-y-0 left-0 z-10 hidden w-7 bg-gradient-to-r from-background via-background/65 to-transparent sm:block"
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-y-0 right-0 z-10 hidden w-7 bg-gradient-to-l from-background via-background/65 to-transparent sm:block"
      />
      <div className="no-scrollbar flex overflow-x-auto pb-2">
        <div aria-hidden="true" className="shrink-0 basis-4 sm:basis-5" />
          ...
          return (
            <AnimatedSection key={link.label} delay={index * 0.05}>
              <a
                ...
                className={`block h-full w-[320px] shrink-0 ${index > 0 ? "ml-4" : ""} ${isGuitar ? "group" : ""}`}
                target={isExternal ? "_blank" : undefined}
                rel={isExternal ? "noreferrer" : undefined}
              >
                <Card
                  className={`h-full min-h-[108px] overflow-hidden transition-colors ${isGuitar ? "relative isolate" : ""}`}
                >
                  {isGuitar ? (
                    <>
                      <div
                  ...
                  <CardContent className="relative z-10 flex h-full items-center justify-between gap-4 p-6">
                    <div className="flex min-w-0 items-center gap-4">
                      <div className="flex size-11 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03] text-foreground">
